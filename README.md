...
<details>
  ...
  - Добавлена команда `/check_plots_dirs` - проверяет все пути, добавленные в чиа. Если по какому-то пути нету плотов, удаляет его из чиа. Ищет на всех дисках подходящих под конфиг `MIN_DISK_TOTAL` с глубиной субдиректорий `DEPTH:` наличие плотов. Добавляет недостающие пути с плотами в чиа.
