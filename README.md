...
<details>
  ...
  - Подключение к MQTT и публикация параметров фермы(температуры, использование памяти, скорость вращения вентиляторов GPU, хэшрейт) в топик `ferma/node<number of node>`
  ...
  - контроль SMART параметров дисков, уведомление при появлении ошибок
</details>
<details>
  <summary>1.9</summary>

  - Есть подозрение что частый запрос SMART для контроля температуры через `smartctl(smartmontools)` на некоторых моделях дисков приводит к остановке засева на третьей фазе. Теперь запрос температуры происходит через  `hddtemp`. Запрос SMART параметров происходит один раз в сутки в 10:00
